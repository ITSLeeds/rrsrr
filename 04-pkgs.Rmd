...
Packages are useful because they enhance the range of things you can do with R.

Packages provide additional functions, data and documentation.
They range from general-purpose packages such as the `tidyverse` and `sf` to domain-specific packages such as `stats19`.
In this chapter we will learn about both types of package, with reference to the general purpose package `ggplot2` --- which is part of the tidyverse suite of packages --- and the domain-specific package `stats19`, the purpose of which is to download and clean road traffic casualty data from the UK.
Domain-specific packages are often written by subject-matter experts, providing tried and tested solutions in a particular specialism.
This book was developed partly in response to the `stats19` package, described in the next section.
...
There are three main stages to using a package:
...
Of these the third stage takes by far the most amount of time but the other stages are equally important: you cannot properly use a package unless it has been properly installed, loaded and, to get the best performance out of the latest version, updated when new versions are released.
