...
There might be some mismatch in the records when we pull STATS19 data as it is the case in this example, and we have raised this with Department For Transport safety team.  
```{r}
sum(ac$number_of_casualties) == nrow(ca) # F
sum(ac$number_of_vehicles) == nrow(ve)   # F
```
The reason for the mismatch in the tables is that some of the "accident-index" values within casualty or vehicle tables are not within the accidents table. We know the three tables come from one STATS19 form and so all records in the other two tables should be indexed in the accidents table. We can make sure there is no mismatch between the tables before we proceed:
...
check = function(year = 2019) {
  ...
  a = get_stats19(year = year, type = "ac", output_format = "sf")
  c = get_stats19(year = year, type = "ca")
  v = get_stats19(year = year, type = "ve")

  # all(c$accident_index %in% a$accident_index)
  # all(v$accident_index %in% a$accident_index)

  caNotINac = which(!c$accident_index %in% a$accident_index)
  veNotINac = which(!v$accident_index %in% a$accident_index)
  acNotINca = which(!a$accident_index %in% c$accident_index)
  acNotINve = which(!a$accident_index %in% v$accident_index)
  v = list(caNotINac, veNotINac, acNotINca, acNotINve)
  names(v) = n
  return(v)
}
# here we only check 2018 and 2019 records
checks = lapply(2018:2019, function(x) check(x))
...
names(d) = 2018:2019
rownames(d) = n
kableExtra::kable(d, caption = "Cross checking 'accident_index' between the three tables")
```

Therefore, we can safely update the tables and run some further checks to make sure that records in the accidents table match with those in the now updated other two tables:
```{r}
