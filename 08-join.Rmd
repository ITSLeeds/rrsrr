...
There might be, and we have raisd this with Department For Transport safety team some mistamch in the records.  
```{r}
sum(ac$number_of_casualties) == nrow(ca) # F
sum(ac$number_of_vehicles) == nrow(ve)   # F
```
The reason for slight mismatch in the join table is some of the "accident-index" values within casualty or vehicle tables are not within the accidents table. We know the three tables come from one STATS19 form and we can make sure there is no mismatch between the tables before we proceed any further:

```{r}
n = c("caNotINac","veNotINac","acNotINca", "acNotINve")
# check accidents table for NA in index
if(any(is.na(ac$accident_index))) { # should be FALSE
  ac = ac[!is.na(ac$accident_index), ]
}
...
df = list(length(caNotINac), length(veNotINac), length(acNotINca), 
          length(acNotINve))
names(df) = n
kableExtra::kable(df)
```

Therefore, we safely update the tables and run some further lines to make sure that records in the accidents table match with those in the newly updated records:
```{r}
