...
Time is ubiquitous in road safety data: collisions and road safety implementations always happen at some point in time.
This chapter will show how you can analyse the temporal dimensions of the real world `crashes_2019` object we created in Chapter \@ref(pkgs) and then demonstrate how to handle time series data in base R and with `hms` and `lubridate` packages
...
If you are the kind of person who likes to know the details first, feel free to skip the next sections and return to the first section later.
...
To consolidate the plotting know-how learned in Chapter \@ref(pkgs), let's start by simply plotting the `datetime` object. Plotting data is a good way to understand new datasets and the variables they contain.
...
The third plot, which makes use of `ggplot2`'s functionality to show only certain labels on the Y axis and reduced opacity so that overlapping points are not completely black, is by far the most useful.
...
The result is useful, but if we're interested in the number of crashes per hour on different days of the week *relative to the average*, we need to undertake more preprocessing steps.
...
1. Look at the help page of the `lubridate` function `month` to see how it is possible to extract the current month as character vector 
...
If the Hour data does not specify the seconds, we just a subfunction of `hms`, namely `hm`, to get the hours and minutes rather than hours, minutes and seconds. 
...
1. **Bonus (difficult):** run the following code, which downloads data for car crashes that occurred during 2017.
