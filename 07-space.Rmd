...
1. Plot the geometry of the zones, with the zones containing crashes overlaid on top in red (see Figure \@ref(fig:sp-ex) left).
1. Plot the zone containing the 2^nd^ crash in blue (see Figure \@ref(fig:sp-ex) middle).
1. **Bonus:** plot all zones that intersect with a zone containing crashes, with the actual crash points plotted in black(see Figure \@ref(fig:sp-ex) right).
...
With `sf` objects it works as follows, which joins the data from the `crashes_sf` dataset onto the 'target' `zones` dataset, to create a new object called `zones_joined`:
...
1. Plot the `casualty_age` variable of the new `zones_joined` object (see Figure \@ref(fig:joinf), left, to verify the result).
1. How many zones are returned in the previous command? 
1. Select only the `geo_code` column from the `zones` and the `dark` column from `crashes_sf` and use the `left = FALSE` argument to return only zones in which crashes occurred. Plot the result (it should look like the map shown in Figure \@ref(fig:joinf), right).
...
A geographic system, such and Longitude/Latitude, refers to locations on the 3D surface of the globe.
Distance and direction calculations work differently between geographic and projected CRSs, so it it often necessary to convert from one to another.
...
1. Create an interactive version of the `tmap` plot by setting `tmap_mode("view")` and re-running the plotting commands.
...
3. **Bonus:** Use the `aggregate()` function to identify how many crashes happened per segment and plot the result (hint: see `?aggregate.sf` and take a read of Section [4.2.5](https://geocompr.robinlovelace.net/spatial-operations.html#spatial-aggr) of Geocomputation with R) with `tmap` and plot the crashes that happened outside the road buffers on top, as shown in Figure \@ref(fig:iowbuff).
1. Try undertaking the same steps for a different region of your choice. Example for Exeter shown in the code chunk which results in Figure \@ref(fig:ex), thanks to Will Cubbin).^[
...
# "https://github.com/saferactive/rrsrr/releases/download/v0/roads.Rds"
```

```{r ex, echo=FALSE, fig.cap="Roads in Essex downloaded with the code shown above."}
ur = "https://github.com/saferactive/rrsrr/releases/download/v0/roads.Rds"
