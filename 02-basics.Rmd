...
The same applies to data science: it will help to understand a little about syntax and grammar of the language that we will use to do things with data (the statistical programming language R) before diving into using it for road safety research.
This chapter may seem tedious for people who just want to crack on and load-in data.
However, working through the examples below is recommended for most people unless you're already an experienced R user, and even experienced R users may learn something about the language's syntax and fundamental features.
...
```{r console, echo=FALSE, fig.cap='The R console pane in RStudio that appears when you first open RStudio. After typing install.packages(stats), you should see the package name auto-completion. Pressing Enter at this point will trigger the autocompletion of the command `install.packages("stats19").'}
...
This is all well and good, providing a chance to see how R works with numbers and to get practice with typing commands into the console.
However, the code chunks above do not make use of a key benefit of R: it is *object oriented*, meaning it stores values and complex objects such as data frames representing road casualties and processes them *in memory* (meaning that R is both fast and memory hungry when working with large datasets).
If you are more familiar with Excel, a data frame may be thought of as fulfilling the purpose of a single spreadsheet containing a set of data.
...
The objects created in the previous code chunk have now served their purpose (to demonstrate basic object creation in R) so, based on wise saying that tidying up is the most important part of job, we will now remove these objects:
...
The final stage in the previous section was to create two objects with sensible names in our R session.^[
...
You should be able to see the object names in the Environmnet tab in the top right of RStudio.
...
This helps explain the command `rm(list = ls())`, which removed all objects in in the global environment in the previous section.
...
To convert a series of vectors into a data frame, of the type shown in an Excel spreadsheet, with rows and columns, we will use the `data.frame()` function.
...
This prints its the contents (results not shown, you need to run the command on your own computer to see the result):
...
As we saw above, the most basic type of R object is a *vector*, a sequence of values of the same type such as the numbers in the object `casualty_age`. 
...
Subsetting means 'extracting' only part of a vector or other object, so only the parts of most interest are returned to us.
Subsets of vectors can be returned by providing numbers representing the positions (index) of the elements within the vector we (e.g. '2' representing selection of the 2^nd^ element) or with logical (`TRUE` or `FALSE`) values associated with the element.
Both of the following commands return the 2nd element of the `casualty_age` object, for example:
