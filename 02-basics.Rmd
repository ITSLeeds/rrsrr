...
However, a key benefit of R is that it is *object oriented*, meaning it stores values and complex objects such as data frames representing road casualties and processes them *in memory* (meaning that R is both fast and memory hungry when working with large datasets). If you are more familiar with Excel, a data frame may be thought of as fulfilling the purpose of a single spreadsheet containing a set of data.
...
Without looking at its contents, we can get a handle of what is in the `crashes` object as follows:
