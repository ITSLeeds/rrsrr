...
Although you could just dive in and start gesticulating to people in central Paris, it's worth taking time to understand the structure and key words of the language first.
...
If you are more familiar with Excel, a data frame may be thought of as fulfilling the purpose of a single worksheet containing a set of data.
...
The objects created in the previous code chunk have now served their purpose (to demonstrate basic object creation in R) so, based on the wise saying that tidying up is the most important part of a job, we will now remove these objects:
...
These methods are demonstrated below to return the 2nd element of the `casualty_age` object, for example:
...
Subsetting in base R is done by using square brackets `[]` after the name of an object.
