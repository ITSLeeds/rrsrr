...
## Subsetting by index or name CHANGE TITLE OR BE CLEARER WHEN YOU ARE SUBSETTING BY INDEX OR NAME IN THE TEXT BELOW

Subsetting returns part of an R object. 
It can be done by providing numbers representing the positions of the elements we want (e.g. the 2^nd^ element) or with a logical vector *NEED TO EXPLAIN VECTOR, with values associated with `TRUE` or `FALSE` returned. 
...
**Run the following commands to practice subsetting by position and verify that you get the same results that are shown below.**
...
There are many ways to recode values in R, the simplest and most mature of which is the use of factors DEFINE WHAT A FACTOR IS, as shown below:
...
You can also save individual R objects in the RDS format. The RDS format is the ? data format for R. (? is this the right term?)
